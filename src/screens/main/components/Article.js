...
const Article = ({ setCitiesOnMap, useAutoSelection }) => {
  ...
  // Перевірка на наявність значення в контексті
  if (!contextValue) {
    return <div>Завантаження...</div>;  // Якщо контекст не ініціалізовано, показуємо повідомлення про завантаження
  }

  const [selectedCities, setSelectedCities, messageReducer] = contextValue;  // Розпаковка значень з контексту
  ...
  const selectAllHandler = async () => {
    ...
    messageReducer(`${message.successes} has been successfully added, ${message.errors} were unsuccessfully added, ${message.alreadyExists} already exists in the table`,
  ...
  const selectByNameHandler = async () => {
    ...
    try {
      ...
      messageReducer("1 city has been successfully added to the table", false);
    }
    catch (error) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!(errors === 0 && success === 0)) {
        messageReducer(`${success} cities have been successfully added to the table, ${errors} were unsuccessfully added`, (errors == 0));
      }
    })()
  }, []);
